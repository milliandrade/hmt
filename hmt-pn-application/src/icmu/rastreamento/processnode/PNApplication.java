...
//				System.out.println(String.format("Sending |%s| to %s", messageText, Constants.GROUND_STATION_UUID));
//			}
...
//							track_id = this.getFarm().getGrazes().get(this.farm.getGrazes().size() - 1).getId();
//							this.getFarm().getGrazes().get(this.farm.getGrazes().size() - 1).setEnd(null);
//						}
...
//										System.out.println(i + "::ox received: " + msgReceived[i]);
										if (!tmpGraze.hasPresentID(msgReceived[i])) {
											...
											this.getFarm().getQuery().executeUpdate(tmpGraze.insertPresentIDtoBD(msgReceived[i]));
										}
